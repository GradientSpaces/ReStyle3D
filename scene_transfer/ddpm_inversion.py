...
def invert(x0, pipe, prompt_src="", num_diffusion_steps=100, cfg_scale_src=3.5, eta=1, depth=None):
    ...
    wt, zs, wts = ddpm_inversion(pipe, w0, etas=eta, prompt=prompt_src, cfg_scale=cfg_scale_src,
                                            prog_bar=True, num_inference_steps=num_diffusion_steps, depth=depth)
    return zs, wts


def ddpm_inversion(model, x0,
                              etas=None,
                              prog_bar=False,
                              prompt="",
                              cfg_scale=3.5,
        ...
        with torch.no_grad():
            ...
            if not prompt == "":
                if depth is not None:
                    ...
                    cond_out = model.unet(xt, timestep=t, encoder_hidden_states=text_embeddings)

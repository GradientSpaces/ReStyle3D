import math
import torch
from scene_transfer import OUT_INDEX
...
def compute_scaled_dot_product_attention(Q, K, V, edit_map=False, is_cross=False, contrast_strength=1.0, masks=None):
    """ Compute the scale dot product attention, potentially with our contrasting operation. """
    cost_volume = Q @ K.transpose(-2, -1) / math.sqrt(Q.size(-1))
    # attn_weight = torch.softmax((Q @ K.transpose(-2, -1) / math.sqrt(Q.size(-1))), dim=-1)
    if masks is not None:
        mask_64, mask_32 = masks
        if (Q.shape[-2] == 32 ** 2):
            cost_volume[OUT_INDEX] = cost_volume[OUT_INDEX] * mask_32
            
        if (Q.shape[-2] == 64 ** 2):
            cost_volume[OUT_INDEX] = cost_volume[OUT_INDEX] * mask_64
            
    attn_weight = torch.softmax(cost_volume, dim=-1) 

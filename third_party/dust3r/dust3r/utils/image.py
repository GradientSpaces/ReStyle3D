...
def load_images_1024(folder_or_list, size, square_ok=False):
    """ Open and convert all images in a list or folder to proper input format,
    resizing them to specified size without cropping.
    
    Parameters:
    folder_or_list (str or list): Path to folder containing images or list of image paths
    size (int): Target size (224 or 512) for resizing
    square_ok (bool): Flag for square images handling (not used in this version)
    ...
    for path in folder_content:
        ...
        img = exif_transpose(PIL.Image.open(os.path.join(root, path))).convert('RGB')
        img_ori = img  # Save original image
        W1, H1 = img.size
        
        # Resize based on size parameter (following original logic but without cropping)
        if size == 224:
            # Resize short side to 224
            if H1 < W1:
                new_H = 224
                new_W = int(W1 * (new_H / H1))
            else:
                new_W = 224
        ...
        # Create dictionary with processed image and metadata
        imgs.append({
            'img': ImgNorm(img)[None],  # Normalize and add batch dimension
            'true_shape': np.int32([img.size[::-1]]),  # [H, W]
            'idx': len(imgs),
            'instance': str(len(imgs)),
            'img_ori': ImgNorm(img_ori)[None]  # Original image
        })
